<?php
class PagSeguro{
	private $email         = "purcinelli_k19@hotmail.com";
	private $token_sandbox = "5E53502523F6411AA4A544FE794F4C82";//AREA DE TESTE PAGAMENTOS NÃO SÃO EFETIVADOS
	private $token_oficial = "TOKEN DO PAGSEGURO";
	private $url_retorno   = "http://SEUSITE/pagseguro/notificacao.php";//qUANDO OCORRER BAIXA BANCARIA É POR AQUI QUE O PAGSEGURO VAI INFORMR
	
	//URL OFICIAL => ATENÇÃO AQUI!!!!!!!!!!
	...
	*/
	...
	public function __construct(){
		$this->email_token = "?email=".$this->email."&token=".$this->$token_sandbox; //PARA TESTES MUDAR AQUI DO TOKEN OFICIAL PARA O SANDBOX!!!!!
	...
	public function getStatusByCode($code){
		...
		if($transaction == 'Unauthorized') {
			...
			//sugiro enviar um e-mail avisando para alguém fazer a manutenção => cÓDIGO DE ERRO OU EMAIL OUOS DOIS????
	...
	//Obtém o status de um pagamento com base na referência (DEFINIDA POR NÓS, EXEMPLO: PEDIDO 10, OU PODE SER O ID DO BANCO DE DADOS)
	//Se o pagamento existir, retorna um código de 1 a 7
	//Se o pagamento não exitir, retorna NULL
